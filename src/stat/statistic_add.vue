<template lang="pug">
  .page
    ...
    button.weui-btn(@click="addInvitation" type="default" open-type="share" :disabled = "disabled") 发起统计
</template>
    ...
    //
      ...
      this.$http.get('invitation/addInvitation', {
        ...
        if (d.data.state === 'success') {
          ...
          that.myinvitations[that.date.split('-')[1]].push({
            time: that.time,
            date: that.date,
            month: that.date.split('-')[1],
            thing: that.thing,
            place: that.place,
            eventKey: '',
            invitee: ''
