<template lang="pug">
  .page
    ul.weui-cells
      li.weui-cell(v-for="stat in statistics[month]")
        label 标题 {{stat.title}}
        label 事件：{{stat.thing}}}
        label 地点：{{stat.place}}
    ...
    ul.weui-cells
      li.weui-cell(v-for='choice in statistics[month].choices' :key="choice.number" @click="click(choice)")
        div
          label {{choice.date}}
          label {{choice.time}}
          image(v-if='reply[choice.number]' src="'/static/choose.png")
